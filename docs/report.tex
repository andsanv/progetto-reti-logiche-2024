...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        ...
        \node[state] (set_read_data_state) {SET_READ_DATA};
        \node[state, initial, right of=set_read_data_state] (reset_state) {RESET};
        \node[state, below of=reset_state] (ready_state) {READY};
    \end{tikzpicture}
